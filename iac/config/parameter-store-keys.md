...
| `/slack-dishes/signing-secret`         | SecureString            | Slack Signing Secret (from app Basic Information). Required for Slack Events API endpoint (SlackEvents Lambda) to verify requests. Slack app Event Subscriptions: **message.im** (ảnh thread DM menu), **message.channels** (+ **message.groups** nếu kênh private). Control channel: tin `POST: …` → gửi vào `channel-id`. Menu channel thread: @Mr.Chef → CollectOrders. |
...
| `/slack-dishes/dishes-sheet-name`      | String (optional)       | Tab sheet cho **orders** (không còn ghi danh sách món). Mặc định: `Tháng {month} / {year}` (GMT+7). Nếu không set (prod hiện tại): **ensure-month-sheet** + CollectOrders/Zalo tự tạo tab tháng mới bằng cách dup tab `Tháng N / YYYY` gần nhất rồi clear data đặt món.                                                                                                                                                                                                                           |
