...
| -------------------------------------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/slack-dishes/bot-token`              | SecureString            | Slack Bot User OAuth Token (xoxb-...). Scopes: `chat:write`, `reactions:read`, `reactions:write`, `users:read`, `im:write`, `im:history`, `files:read`, **`files:write`** (upload ảnh Zalo → Slack; `chat.update` menu).                                                                                   |
...
| `/slack-dishes/menu-dm-user-id`        | String (optional)       | Slack user ID whose **latest DM message** is used as menu source (mỗi dòng không rỗng = một món). Default: `U02SJRNAM2M`.                                                                                                                                                                                       |
...
| `/slack-dishes/zalo-group-id`          | String (optional)       | Zalo **group** id. Bắt buộc cho **ZaloSheetSummary** (11h) và **ZaloMenuImages** (poll ảnh). `node scripts/zalo/list-groups.mjs` |
| `/slack-dishes/zalo-menu-source-user-id` | String (optional)     | Zalo **uid** nhà bếp. Khi set: poll ảnh từ sau POST_MENU đến trước ZALO_SUMMARY (10 phút/lần). Lấy uid: `node scripts/zalo/inspect-group-images.mjs <groupId>`. |
| `/slack-dishes/zalo-menu-history-count` | String (optional)    | Số tin group đọc mỗi lần poll (mặc định `50`). |
...
**Ghi chú:** Cửa sổ poll ảnh = `POST_MENU` + 5 phút → trước `ZALO_SUMMARY` (`serverless/shared/time-constants.js`). Đổi giờ menu (vd. 9h): sửa `POST_MENU_HOUR_GMT7` + cron PostMenu trong `iac/template.yaml`; nếu menu &lt; 9h mở rộng giờ UTC trong cron `zalo-menu-images-poll`.
...
# aws ssm put-parameter --name /slack-dishes/zalo-menu-source-user-id --value "KITCHEN_ZALO_UID" --type String
