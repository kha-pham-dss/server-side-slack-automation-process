...
- **slack-events** – HTTP endpoint (Lambda Function URL). **Control channel:** tin `POST: …` → invoke **control-channel-post** (gửi nội dung vào `channel-id`, ✅ trên tin control). Reply dưới menu hôm nay + @Mr.Chef → collect-orders. Subscribe **message.channels** / **message.groups**.
